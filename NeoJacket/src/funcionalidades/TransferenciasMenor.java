...
/**
 * Equivalente de funcionalidades.Transferencias pero para cuentas de menores
 * supervisados (gui.TransferenciasMenor). Misma lógica de negocio (búsqueda
 * de cuentas, tipo de cambio, ejecución real en BD), con un paso adicional:
 * antes de ejecutar la transferencia se valida el límite de gasto aprobado
 * por el tutor (SupervisionDAO.validarLimite).
 */
public class TransferenciasMenor {

    private final gui.TransferenciasMenor vista;
    private final Integer idMenor;
    ...
    public TransferenciasMenor(gui.TransferenciasMenor vista) {
        this.vista = vista;
     ...
     * Permite ingresar tanto el Número de Cuenta como el Número de Tarjeta.
     */
    ...
    /**
     * BLOQUE 2: Busca los datos de destino. No importa si está registrada o
     * no en la BD, la transferencia continúa igual (cuenta externa).
     */
    ...
    /**
     * Actualiza de manera automática el resumen gráfico inferior conforme
     * cambian los datos de la interfaz.
     */
    ...
    private void ejecutarTransferencia() {
        ...
        // --- VALIDACIÓN ESPECÍFICA DE CUENTA SUPERVISADA ---
        // Se ejecuta antes de tocar la base de datos: si el menor excede su
        // límite de gasto aprobado, se bloquea la transferencia aquí mismo.
        if (idMenor != null) {
            SupervisionDAO daoSup = new SupervisionDAO();
            String errorLimite = daoSup.validarLimite(idMenor, monto);
            if (errorLimite != null) {
                JOptionPane.showMessageDialog(vista, errorLimite, "Límite de gasto excedido", JOptionPane.WARNING_MESSAGE);
                return;
            }
        }
        ...
        try {
            ...
            // 2. Acreditar a la cuenta destino (solo si es una cuenta interna registrada)
            ...
            // 3. Registrar auditoría en la tabla transacciones
            ...
            try (PreparedStatement psTrans = con.prepareStatement(sqlTransaccion)) {
                ...
                psTrans.setString(8, "Transferencia Digital - Modulo Neo Jacket (Cuenta Supervisada)");
